...
main = do
  ...
  case command of
    RunCommand portNr -> do
      ...
      updateGlobalLogger "{{ cookiecutter.package_name }}" (setLevel DEBUG)
      h <- fileHandler "logs/server.log" DEBUG >>= \lh -> return $ setFormatter
        lh
        (simpleLogFormatter "[$time : $loggername : $prio] $msg")
      updateGlobalLogger "{{ cookiecutter.package_name }}" (addHandler h)
